...
const digest::ClassfileRow *GraphTask::getClassfileRow(const JarEntry &jarEntry) const {
...
bool GraphTask::start() {
    ...
    _jarFileRow = const_cast<digest::FileRow*>(getJarFileRow(filename));
...
void GraphTask::processEntry(const JarEntry &jarEntry, std::mutex &taskMutex) {
    ...
    const std::optional<digest::columns::DigestCol> classDigest =
        isUnchanged
            ? row->digest
...
const digest::FileRow *GraphTask::createJarFileRow(const std::string &filename) const {
    //TODO
...
const digest::FileRow *GraphTask::getJarFileRow(const std::string &filename) {
    //TODO
...
optional<digest::columns::DigestCol> GraphTask::parseEntry(const JarEntry &jarEntry,
                                                              const digest::ClassfileRow *row) const {
    optional<digest::columns::DigestCol> result;
...
bool GraphTask::isClassfileUnchanged(const JarEntry &jarEntry, const digest::ClassfileRow *classRow) {
