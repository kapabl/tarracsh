...
class ClassFileParser {
    ...
    public:
        ClassFileParser(string fileName) : _fileName(fileName), _bytesRead(0) { processFile(); }
    ...
    private:
        vector<string> _accessModifiers = {"PUBLIC",       "PRIVATE",    "PROTECTED", "STATIC",   "FINAL",
                                           "SYNCHRONIZED", "SUPER",      "VOLATILE",  "BRIDGE",   "TRANSIENT",
        ...
        void outputAsInterface() {
            cout << endl;
            cout << "//Interface " << endl;
            cout << "//------------------------------------" << endl;
            outputAccessModifiers(_mainClassInfo.accessFlags);

            wcout << L"interface " << getClassInfoName( _mainClassInfo.thisClass );

            if (_mainClassInfo.superClass != 0) {
                wcout << L" extends " << getClassInfoName( _mainClassInfo.superClass );
            }

            // TODO extends interfaces
        }
        ...
        void outputAsClass() {
            ...
            wcout << L"class " << getClassInfoName( _mainClassInfo.thisClass );
